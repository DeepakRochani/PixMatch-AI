...
export default function ProjectProfitabilityPage() {
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const res = await fetchApi(`/finance/reports/project-profitability?fromDate=${fromDate}&toDate=${toDate}`);
      if (res && !res.error) {
        setReport(res.data || res);
      }
    } catch (err: any) {
      setError(err.message || 'Failed to load Project Profitability report');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExportCSV = () => {
    ...
    window.open(`/api/v1/finance/reports/export/csv?type=PROJECT_PROFITABILITY&fromDate=${fromDate}&toDate=${toDate}&token=${token}&studioId=${studioId}`, '_blank');
  };

  const handleExportPDF = () => {
    const token = typeof window !== 'undefined' ? localStorage.getItem('pixmatch_token') : '';
    const studioId = typeof window !== 'undefined' ? localStorage.getItem('pixmatch_studio_id') : '';
    window.open(`/api/v1/finance/reports/export/pdf?type=PROJECT_PROFITABILITY&fromDate=${fromDate}&toDate=${toDate}&token=${token}&studioId=${studioId}`, '_blank');
  };
  ...
  const filteredProjects = report?.projects?.filter((p: any) =>
    p.project_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    p.client_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    p.shoot_type?.toLowerCase().includes(searchTerm.toLowerCase())
  ) || [];
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-white flex items-center gap-2.5">
            ...
            Project & Shoot Profitability Analytics
          </h1>
          <p className="text-sm text-muted mt-1">
            Job costing, direct labor, gear costs, net project margins, and profitability attribution across shoot categories.
          </p>
        </div>
      ...
      <div className="p-4 rounded-xl bg-card-bg border border-card-border flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search project, client, or shoot type..."
            value={searchTerm}
            ...
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="border-b border-card-border text-muted uppercase text-[10px]">
                  <th className="py-3 px-3">Project / Shoot</th>
                  <th className="py-3 px-3">Client</th>
                  <th className="py-3 px-3">Type</th>
                  <th className="py-3 px-3 text-right">Invoiced Revenue</th>
                  <th className="py-3 px-3 text-right">Direct Costs</th>
              ...
              <tbody className="divide-y divide-card-border/30">
                {filteredProjects.map((p: any) => (
                  <tr key={p.project_id} className="hover:bg-card-border/20 transition">
                    <td className="py-3 px-3">
                      <div className="font-semibold text-white">{p.project_name}</div>
                      <div className="text-[10px] text-muted">{p.status}</div>
                    </td>
                    <td className="py-3 px-3 text-muted">{p.client_name || '—'}</td>
                    <td className="py-3 px-3 text-muted capitalize">{p.shoot_type || 'General'}</td>
                    <td className="py-3 px-3 font-mono text-right text-white font-semibold">
                      {formatCurrency(p.revenue, report.currency)}
                    </td>
                    <td className="py-3 px-3 font-mono text-right text-rose-300">
                      {formatCurrency(p.direct_costs, report.currency)}
                    </td>
                    <td className="py-3 px-3 font-mono text-right font-bold text-emerald-400">
                      {formatCurrency(p.net_profit, report.currency)}
                    </td>
                    <td className="py-3 px-3 font-mono text-right">
                      <span className={`px-2 py-0.5 rounded text-[11px] font-semibold ${
                        p.margin_percentage >= 50 ? 'bg-emerald-500/10 text-emerald-400' :
                        p.margin_percentage >= 25 ? 'bg-cyan-500/10 text-cyan-400' :
