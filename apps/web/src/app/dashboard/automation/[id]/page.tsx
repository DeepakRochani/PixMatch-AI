...
export default function EditWorkflowPage() {
    ...
    const loadWorkflow = async () => {
      try {
        ...
        if (config.failureHandling) {
          setRetryFailedStep(config.failureHandling.retryFailedStep !== false);
          setMaxRetries(config.failureHandling.maxRetries || 3);
          setContinueOnFailure(!!config.failureHandling.continueOnNonCriticalFailure);
          setNotifyOnFailure(config.failureHandling.notifyOnFailure !== false);
        }
      } catch (err: any) {
  ...
  const handleSave = async () => {
    ...
    const payload: UpdateAutomationWorkflowDTO = {
      name,
      description,
      enabled,
  ...
  if (isLoading) {
    return <div className="p-8 text-center text-muted">Loading workflow details...</div>;
  }
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-8">
      ...
      <div className="flex items-center justify-between border-b border-card-border pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">Edit Workflow: {name}</h1>
            <p className="text-xs text-muted">Modify triggers, execution steps, and approval rules.</p>
          </div>
        </div>
      ...
      <div className="p-6 rounded-2xl bg-card border border-card-border space-y-5">
        <h3 className="font-semibold text-white text-base">1. Workflow Configuration</h3>

        <div className="space-y-4">
          <div className="flex items-center justify-between p-3 rounded-xl bg-[#0E131F] border border-card-border">
            <div>
              <div className="text-xs font-semibold text-white">Workflow Active Status</div>
              <div className="text-[11px] text-muted">Enable or disable automatic trigger execution</div>
            </div>
            <input
              type="checkbox"
              checked={enabled}
      ...
      <div className="p-6 rounded-2xl bg-card border border-card-border space-y-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted">Order and define safe automation operations.</p>
          </div>
          <button
