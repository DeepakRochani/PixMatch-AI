...
import { Activity, ArrowLeft, ShieldAlert, Eye, RefreshCw } from 'lucide-react';
...
function ApiSecurityContent() {
  ...
  const fetchApiEvents = async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/security-center/api');
      if (res.ok) {
        const d = await res.json();
        if (d.success) setEvents(d.data.events || []);
      }
    } catch (e) {
      console.error('Failed to load API security events:', e);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-card-border/80 pb-5">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold tracking-tight text-white">API & Rate Limit Abuse</h1>
          </div>
          <p className="text-xs text-muted mt-1">
            Detection of IDOR authorization bypasses, aggressive endpoint scraping, and rate limit exhaustion.
          </p>
        </div>

        <button
          onClick={fetchApiEvents}
              ...
              {loading ? (
                <tr>
                  <td colSpan={7} className="p-8 text-center text-muted">
                    Loading API security events...
                  </td>
                </tr>
              ) : events.length === 0 ? (
                <tr>
                  <td colSpan={7} className="p-8 text-center text-muted">
                    No API abuse or authorization violations recorded.
                  </td>
                </tr>
              ) : (
                events.map((ev) => (
...
export default function ApiSecurityPage() {
  return (
    <Suspense fallback={<div className="p-8 text-center text-muted text-xs">Loading API Security...</div>}>
      <ApiSecurityContent />
    </Suspense>
  );
}
